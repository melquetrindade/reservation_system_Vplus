...
import {
  AddHorarioForm,
  addHorarioSchema,
} from "@/app/schema/add-horario-schema";
...
interface ShowSchedulesAgendaProps {
  ...
  setDisponibilidade: Dispatch<
    SetStateAction<DisponibilidadeComAgendamentos[]>
  >;
  disponibilidade: DisponibilidadeComAgendamentos[];
  professionalId: string;
}

const ShowSchedulesAgenda = ({
  selectedDay,
  disponibilidade,
  setDisponibilidade,
  professionalId,
}: ShowSchedulesAgendaProps) => {
  const [selectedSlot, setSelectedSlot] =
    useState<DisponibilidadeComAgendamentos | null>(null);
  ...
  const descriptionDialog = (slot: DisponibilidadeComAgendamentos) => {
    if (slot.status === "RESERVADO") {
      return (
        <div className="p-4 flex flex-col items-start">
          <p>
            ...
            {slot.agendamentos[0]?.usuario?.name}
          </p>
          <p>
            <span className="font-semibold">Telefone: </span>
            {slot.agendamentos[0]?.usuario?.telefone}
          </p>
          <p>
            <span className="font-semibold">E-mail: </span>
            {slot.agendamentos[0]?.usuario?.email}
          </p>
        </div>
      );
      ...
      return (
        <p>
          Ao bloquear, seus clientes não poderão reservar este horário. Você
    ...
    return (
      <p>
        Ao desbloquear, seus clientes poderão reservar este horário. Você pode
  ...
  const handleBlock = async (slot: DisponibilidadeComAgendamentos) => {
    try {
      ...
      setDisponibilidade((old) =>
        old.map((d) =>
          d.id === slot.id ? { ...d, status: upTime.status as any } : d,
        ),
      );
  ...
  return (
    <div className="px-5 flex flex-col items-center justify-center">
                ...
                <DialogTrigger onClick={() => setSelectedSlot(slot)}>
                  ...
                  >
                    ...
                    <span
                      className={`${slot.status === "LIVRE" ? "text-green-600" : slot.status === "RESERVADO" ? "text-yellow-600" : "text-gray-600"}`}
                    >
                      {format(slot.horaInicio, "HH:mm")} -{" "}
                      {format(slot.horaFim, "HH:mm")}
                    </span>
                  </div>
                </DialogTrigger>
                  ...
                  <Button
                    ...
                    onClick={
                      slot.status === "RESERVADO"
                        ? () =>
                            copyPhone(slot.agendamentos[0]?.usuario?.telefone!)
      ...
      >
        <DialogTrigger>
          <Button className="mb-5 mt-5">Cadastrar novo horário</Button>
        </DialogTrigger>
        <DialogContent className="w-[90%] max-h-[90vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
          <DialogHeader>
            ...
            <DialogDescription>
              Os novos horários não podem conflitar com horários já existentes
              (LIVRE, BLOQUEADO ou RESERVADO).
            </DialogDescription>
          </DialogHeader>

          <form
            onSubmit={form.handleSubmit(handleAddSchedules)}
            className="space-y-6"
          >
            <FieldGroup>
              <Field>
